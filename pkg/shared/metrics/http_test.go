...
func TestHTTPMiddleware_CapturesNon200Status(t *testing.T) {
	...
	srv := m.Middleware(mux)
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/api/v1/fail", nil)
	srv.ServeHTTP(rec, req)
...
func TestHTTPMiddleware_ImplicitOK(t *testing.T) {
	...
	srv := m.Middleware(mux)
	rec := httptest.NewRecorder()
	srv.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/implicit", nil))
...
func TestHTTPMiddleware_UnmatchedPath(t *testing.T) {
	...
	srv := m.Middleware(mux)
	rec := httptest.NewRecorder()
	srv.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/totally/unknown", nil))
