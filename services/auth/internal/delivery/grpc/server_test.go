...
func TestValidateToken(t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		s := NewServer(newTestLogger(), stubUserProvider{}, stubTokenValidator{
			fn: func(ctx context.Context, token string) (models.User, error) {
				return models.User{ID: 42, Role: "user"}, nil
			},
		})
...
func TestGetUser(t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		s := NewServer(newTestLogger(), stubUserProvider{
			byID: func(ctx context.Context, id int64) (models.User, error) {
				return models.User{ID: 7, Email: "a@b", Name: "Bob", Role: "user", Rating: 4.2}, nil
			},
		}, stubTokenValidator{})
...
func TestCheckRole(t *testing.T) {
	t.Run("zero id -> InvalidArgument", func(t *testing.T) {
		s := NewServer(newTestLogger(), stubUserProvider{}, stubTokenValidator{})
		_, err := s.CheckRole(context.Background(), &authv1.CheckRoleRequest{UserId: 0, RequiredRole: "admin"})
		assert.Equal(t, codes.InvalidArgument, codeOf(t, err))
	})
	...
	t.Run("user not found -> NotFound", func(t *testing.T) {
		...
		_, err := s.CheckRole(context.Background(), &authv1.CheckRoleRequest{UserId: 1, RequiredRole: "admin"})
		assert.Equal(t, codes.NotFound, codeOf(t, err))
	})
	...
	t.Run("admin allowed for any role", func(t *testing.T) {
		s := NewServer(newTestLogger(), stubUserProvider{
			role: func(ctx context.Context, id int64) (string, error) { return "admin", nil },
		}, stubTokenValidator{})
		resp, err := s.CheckRole(context.Background(), &authv1.CheckRoleRequest{UserId: 1, RequiredRole: "support"})
		require.NoError(t, err)
		assert.True(t, resp.GetAllowed())
		assert.Equal(t, "admin", resp.GetActualRole())
	})

	t.Run("matching role allowed", func(t *testing.T) {
		s := NewServer(newTestLogger(), stubUserProvider{
			role: func(ctx context.Context, id int64) (string, error) { return "support", nil },
		}, stubTokenValidator{})
		resp, err := s.CheckRole(context.Background(), &authv1.CheckRoleRequest{UserId: 1, RequiredRole: "support"})
		require.NoError(t, err)
		assert.True(t, resp.GetAllowed())
	...
	t.Run("mismatched role forbidden", func(t *testing.T) {
		s := NewServer(newTestLogger(), stubUserProvider{
			role: func(ctx context.Context, id int64) (string, error) { return "user", nil },
		}, stubTokenValidator{})
		resp, err := s.CheckRole(context.Background(), &authv1.CheckRoleRequest{UserId: 1, RequiredRole: "admin"})
		require.NoError(t, err)
