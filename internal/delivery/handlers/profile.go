...
const (
	opHandleGetProfile       = "handlers.HandleGetProfile"
	opHandleUpdateProfile    = "handlers.HandleUpdateProfile"
	opHandleGetPublicProfile = "handlers.HandleGetPublicProfile"
	opHandleUploadAvatar     = "handlers.HandleUploadAvatar"
)
...
func (h *AuthHandlers) HandleGetProfile(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		h.log.ErrorContext(r.Context(), "user id not found in context",
			slog.String("op", opHandleGetProfile),
		)
...
func (h *AuthHandlers) HandleUpdateProfile(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		h.log.ErrorContext(r.Context(), "user id not found in context",
			slog.String("op", opHandleUpdateProfile),
		)
		responser.RespondWithError(w, http.StatusUnauthorized, ErrUnauthorized)
		return
	}
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		h.log.WarnContext(r.Context(), "failed to decode request body",
			slog.String("op", opHandleUpdateProfile),
			slog.String("error", err.Error()),
		)
...
func (h *AuthHandlers) HandleGetPublicProfile(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.log.WarnContext(r.Context(), "invalid user id",
			slog.String("op", opHandleGetPublicProfile),
...
func (h *AuthHandlers) HandleUploadAvatar(w http.ResponseWriter, r *http.Request) {
	h.log.DebugContext(r.Context(), "checking content type",
		slog.String("op", opHandleUploadAvatar),
	...
	if err := r.ParseMultipartForm(MaxUploadSize); err != nil {
		h.log.WarnContext(r.Context(), "parse multipart form error",
			slog.String("op", opHandleUploadAvatar),
			slog.String("error", err.Error()),
		)
	...
	if err != nil {
		h.log.WarnContext(r.Context(), "failed to get file",
			slog.String("op", opHandleUploadAvatar),
			slog.String("error", err.Error()),
		)
