...
func TestHandleCheckout(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		...
		err := json.NewDecoder(rr.Body).Decode(&response)
		assert.NoError(t, err)
		assert.Len(t, response.OrderIDs, 2)
		assert.Equal(t, "Иван", response.Sellers[2].Name)
		assert.Equal(t, "Петр", response.Sellers[3].Name)
	})
	...
	t.Run("Products unavailable error", func(t *testing.T) {
		cartH, mockCart := setupCartHandlers(t)

		mockCart.EXPECT().Checkout(gomock.Any(), int64(1)).
